...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["CallReco", "Alert", "TGT",
                                              "SL", "Exit", "Text"])
...
cash = stock.loc[stock['Segment'] == 'Cash']
cash_st = [row for row in cash['Symbol']]
cash_st.insert(0, '<select>')

fut = stock.loc[stock['Segment'] == 'Future']
fut_st = [row for row in fut['Symbol']]
fut_st.insert(0, '<select>')

com = stock.loc[stock['Segment'] == 'Commodity']
com_st = [row for row in com['Symbol']]
com_st.insert(0, '<select>')
...
def todb(sym, mont, sigl, lp, stopl, tgt, quty, sgmnt):
    ...
    if sgmnt == "Cash":
        ...
        nim.get_session().add(new_txn)
        st.write("call recorded in db")
...
with tab1:
    ...
    if side == "Cash":
        ...
        with col1:
            segment = "Cash"
            stk = st.selectbox("Select the StockName", cash_st)
        ...
        with col1:
            ...
            stk = st.selectbox("Enter the StockName", fut_st)
        ...
        with col1:
            segment = "COMMODITY"
            stk = st.selectbox("Select the StockName", com_st)
            mon = st.selectbox("Month", ['<select>', "Jan", "Feb", "Mar", "Apr", "May", "Jun",
                                         "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"], 0)
            signal = st.selectbox("OrderType", ['<select>', "BUY", "SELL"], 0)
        with col2:
...
with tab2:
    ...
    q = q0.where((NinjaCalls.Date == today) and (NinjaCalls.Closure is None))
    filtered = pd.read_sql(q.statement, nm.get_session().bind)
    mod_cash = filtered.loc[filtered['Segment'] == 'NSECM']
    modstk_cash = [row for row in mod_cash['Symbol']]
    modstk_cash.insert(0, '<select>')

    mod_fut = filtered.loc[filtered['Segment'] == 'NSEFO']
    modstk_fut = [row for row in mod_fut['Symbol']]
    modstk_fut.insert(0, '<select>')

    mod_com = filtered.loc[filtered['Segment'] == 'COMMODITY']
    modstk_com = [row for row in mod_com['Symbol']]
    modstk_com.insert(0, '<select>')
    ...
    if (side == "Home") or (side == "Message"):
        ...
        col1, col2 = st.columns(2)
        with col1:
            stk = st.selectbox("ChooseStock", modlist)
            if stk != '<select>':
    ...
    elif (side == "Future") or (side == "Others"):
        st.write("Alert for index and future is not available as of now")
    # elif side == "Future":
    # updatedb(stk, newsl)
    # to_channel(call)
    # else:
...
with tab3:
    ...
    if (side == "Home") or (side == "Message"):
        ...
        stk = st.selectbox("Choose Stock", modlist)
        if stk != '<select>':
            val = filtered.loc[filtered['Symbol'] == stk]
            t = val['TGT'].iloc[0]
            rate = val['Rate'].iloc[0]
...
with tab4:
    ...
    if (side == "Home") or (side == "Message"):
        ...
        stk = st.selectbox("Stock", modlist)
        if stk != '<select>':
            val = filtered.loc[filtered['Symbol'] == stk]
            s = val['SL'].iloc[0]
            rate = val['Rate'].iloc[0]
...
with tab5:
    ...
    if (side == "Home") or (side == "Message"):
        ...
        col1, col2 = st.columns(2)
        with col1:
            stk = st.selectbox("StockName", modlist)
        with col2:
